...
def getContainerIpAddress(containerName):
    c=dockerpy.Client()
    print "GET CONTAINER IP: "+str(containerName)
    container = c.inspect_container(containerName)
    return container['NetworkSettings']['IPAddress']

def getContainerIpAddressByHostname(hostname):
    c=dockerpy.Client()
...
#DBB ADD
def getContainerId(hostname):
    c=dockerpy.Client()
    containers = c.containers()
    #print containers
    for container in containers:
        contInfo = c.inspect_container(container)
        if (str(contInfo['Config']['Hostname']) == hostname):
            return contInfo['Id']
    return 0
